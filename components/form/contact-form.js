...
const ContactForm = () => {
    ...
    const showSuccessMessage = () => (<div className="alert alert-info" style={{display: error ? '' : 'none'}}>
        {error}
    </div> )

    const showErrorMessage = () => success && (<div className="alert alert-danger">
        Thank you for contacting us!
    </div> )

    const Form = () => (
        <form onSubmit={clickSubmit} style={{paddingTop: '3rem', paddingBottom: '5rem'}}>
            <div className="form-group">
                <label className="lead">Message</label>
                <textarea onChange={handleChange('message')} type="text" className="form-control" value={message}
                          required
                          rows="10"/>
            </div>
            <div className="form-group">
