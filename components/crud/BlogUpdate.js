...
const BlogUpdate = ({ router }) => {
    ...
    const editBlog = e => {
        e.preventDefault();
        setValues({...values, loading: 'Loading...', showUpdateButton: false})
        updateBlog(formData, token, router.query.slug).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error });
            } else {
                setValues({ ...values, title: '', loading: '', success: `Blog titled "${data.title}" is successfully updated` });
                ...
                <div className="col-xl-4">
                    <div>
                        ...
                        <div>
                            ...
                            <ul style={{ overflowY: 'scroll' }}>{showCategories()}</ul>
                        </div>
                        <div>
                            <h5>Tags</h5>
                            <hr />
                            <ul style={{ overflowY: 'scroll' }}>{showTags()}</ul>
                        </div>
                    </div>
                </div>
